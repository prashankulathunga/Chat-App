            ...
            <div className='md:w-3/4 lg:w-2/3 border rounded-xl shadow-2xl flex border-green-950/50 w-full'>
                <div className='left md:w-1/2 w-full h-full md:px-8 p-4'>
                    ...
                    <div className='input-section px-4'>
                        ...
                        <div className="text-300 font-medium">
                            <p>
                                Hello This isa Test work for test
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleRegister}>
                            <fieldset className="fieldset">
                                ...
                                <input type="text" onChange={(e) => setFullName(e.target.value)}
                                       className="input w-full input-neutral" placeholder="John Doe"/>
                                {error.fullName ? (<p className='label text-300 text-red-400'>{error.fullName}</p>) : (
                            ...
                            <fieldset className="fieldset">
                                ...
                                <input type="email" onChange={(e) => setEmail(e.target.value)}
                                       className="input w-full input-neutral" placeholder="johndoe@example.com"/>
                                {error.email ? (<p className='label text-300 text-red-400'>{error.email}</p>) : (
                                    <p className="label text-300 text-white/25">required</p>)
                                }
                            </fieldset>
                            ...
                            <fieldset className="fieldset">
                                ...
                                <input type="password" onChange={(e) => setPassword(e.target.value)}
                                       className="input w-full input-neutral" placeholder="********"/>
                                {error.password ? (<p className='label text-300 text-red-400'>{error.password}</p>) : (
                                    <p className="label text-300 text-white/25">Password must be at least 6 characters
                                        long</p>
                                )}
                            </fieldset>
                            ...
                            <button type='submit'
                                    className="btn btn-xs sm:btn-sm md:btn-md lg:btn-lg bg-green-700/80 mt-4 w-full text-400 md:p-4 p-5">
                                Create an Account
                            </button>
